# JSON File Write Protocol (MANDATORY FOR ALL AGENTS)

## How to Append to an Existing JSON File

**NEVER use Write to overwrite a JSON file that already exists. Always append.**

### Step-by-step procedure

1. **Read the file first** — use the Read tool to load the existing array.
2. **Find the last `id`** — the new questions must continue the sequence (e.g., if the file ends at `"id": 7`, your first new question is `"id": 8`).
3. **Merge in memory** — combine the existing array with the new questions into one array.
4. **Write the full merged array** — use the Write tool once with the complete array (old + new).

### What NOT to do

- Do NOT write only the new questions — that deletes the existing ones.
- Do NOT assume the file is empty because you just generated questions — another agent may have already written to it.
- Do NOT use `id: 1` for new questions if the file already has content.
- Do NOT write JSON via bash heredoc (`cat << 'ENDJSON'`). JSON explanations routinely contain single quotes (e.g., "more likely than not"), which break the heredoc delimiter and corrupt the file. **Always use the Write tool directly.**
...
print(len(d), "existing questions, last id =", d[-1]["id"])
```
If the file has content, your new questions start at `last_id + 1`. If you skip this step and start at id 1, you will destroy all prior work.

### Example

```
Existing file: ids 1–7 (numerical) + ids 8–12 (assertion-reason) = 12 questions
You are adding: 9 enumeration questions

Wrong: Write ids 1–9   ← destroys the existing 12 questions (THIS HAS HAPPENED 3 TIMES)
Correct: Read file → last id = 12 → Write ids 1–21 (full merged array)
```
